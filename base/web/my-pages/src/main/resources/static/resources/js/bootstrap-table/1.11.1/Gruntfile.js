...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        concat: {
            //basic_target: {
            //    src: ['src/<%= pkg.name %>.json', 'src/extensions/**/*.json'],
            //    dest: 'dist/<%= pkg.name %>-all.json'
            //},
            locale_target: {
                src: ['src/locale/**/*.json'],
                dest: 'dist/<%= pkg.name %>-locale-all.json'
            }
        },
        uglify: {
            options: {
            ...
            basic_target: {
                files: {
                    'dist/<%= pkg.name %>.min.js': ['src/<%=pkg.name %>.json'],
                    //'dist/<%= pkg.name %>-all.min.json': ['dist/<%=pkg.name %>-all.json'],
                    'dist/<%= pkg.name %>-locale-all.min.js': ['dist/<%=pkg.name %>-locale-all.json']
                }
            },
            locale_target: {
                files: [{
                    ...
                    src: '**/*.json',
                    dest: 'dist/locale',
                    ext: '.min.json' // replace .json to .min.json
                }]
            },
            extensions_target: {
                files: [{
        ...
        copy: {
            source: {
                ...
                src: ['**/*.json', '**/*.css'],   // copy all files and subfolders
    ...
    grunt.registerTask('docs', 'build the docs', function () {
        ...
        bumpVersion('./src/bootstrap-table.json', version, ' * version: ');
